...
public class Bong extends JApplet implements Runnable, KeyListener {
	...
	public void paint(Graphics g) {
		...
		if (B_flag == 1){   //フラグ起動中のみボール表示
		  offg.fillOval((int)(x - 3), (int)(y - 3), 6, 6);
		}
        if ( Llimit < 40 ) { offg.setColor(Color.BLACK); }
        else { offg.setColor(Color.RED); }
        offg.drawRect(5, (int)(ySize-5), 40, 3);
        offg.fillRect(5, (int)(ySize-5), Llimit, 3);
        if ( Rlimit < 40 ) { offg.setColor(Color.BLACK); }
        else { offg.setColor(Color.RED); }
			...
			while (true) {
				...
				if (dx < 0 && (x - paddleXL) * (x - dx - paddleXL) <= 0) {
					...
					if ((rY - paddleYL + paddleSize / 2) * (rY - paddleYL - paddleSize / 2) <= 0) {
						...
						message = "";
						L_flag = 0;
						if ( Llimit < 40 ) { Llimit += 4; }    //返したらゲージ増
					}
				}
				if (x < 0) {
					x = -x;
	...
	public void keyPressed(KeyEvent e) {
		...
		case 'S':  if ( Llimit == 40) { L_flag = 1; } break;
		case 'K':  if ( Rlimit == 40) { R_flag = 1; } break;
		}
	}
